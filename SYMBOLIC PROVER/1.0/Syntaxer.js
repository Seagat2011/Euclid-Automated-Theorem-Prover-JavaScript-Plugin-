...
DESCRIPTION: 
   ...
   in an attempt to verify their syntax.
...
SCRIPT TYPE: 
   Syntax/Semantics analysis engine
...
var NFA = {}// finite-automata (non-deterministic) //
...
var fileNFA = 'Syntaxer_NFA_settings.js'
var nfa = new Worker(fileNFA)
nfa.addEventListener('message', nfa_init,'Syntaxer.js')
nfa.postMessage({})
...
function SKN(v,b)
{
   this.scan_token = function( i,j ){
      ...
      var k1 = invEN[peek[j].tok]
      var k2 = invEN[peek[(j+1)].tok]
      if(!NFA[ k1 ][ k2 ]){
         var w = []; 
         k1 = 'en.'+k1
         k2 = 'en.'+k2
         w.push('NFA['+k1+']['+k2+'] == 0')
         throw parser['44CDEB54C6F2AEBAD54611201C26D6F0'](i,(j+1),k2,w.join(' or '))
      }
   }
}
...
parser[en.Comment] = new SKN(); parser[en.Comment].scan_token = function(){ var i = ++arguments[0]; var j = ++arguments[1]; if(j==1){ Comment.activate_lineComment() } return j; }
...
parser[en.OpenBracket] = new SKN(); parser[en.OpenBracket].scan_token = function(){ stack.parens++ }
parser[en.CloseBracket] = new SKN(); parser[en.CloseBracket].scan_token = function(){ stack.parens-- }
parser[en.OpenCurly] = new SKN(); parser[en.OpenCurly].scan_token = function(){ stack.braces++ }
parser[en.CloseCurly] = new SKN(); parser[en.CloseCurly].scan_token = function(){ stack.braces-- }
parser[en.OpenSquare] = new SKN(); parser[en.OpenSquare].scan_token = function(){ stack.brackets++ }
parser[en.CloseSquare] = new SKN(); parser[en.CloseSquare].scan_token = function(){ stack.brackets-- }
...
parser[en.FOFBinOp] = new SKN()
   ...
   // LMultilineComment|Comment //
parser[en.LMultilineComment] = new SKN(); parser[en.LMultilineComment].scan_token = function(){ var i = ++arguments[0]; var j = ++arguments[1]; if(!Comment.MultilineComment){ Comment.activate_MultilineComment() }; return j; }
   // RMultilineComment|Comment //
parser[en.RMultilineComment] = new SKN(); parser[en.RMultilineComment].scan_token = function(){ var i = ++arguments[0]; var j = ++arguments[1]; if(Comment.MultilineComment){ Comment.deactivate_MultilineComment() }; return j; }
parser[en.Newline] = new SKN()
parser[en.hashmark] = new SKN(); parser[en.hashmark].scan_token = function(){ var i = ++arguments[0]; var j = ++arguments[1]; if(j==1){ Comment.activate_lineComment() } return j; }
...
parser[en.range] = new SKN()
parser[en.domain] = new SKN()
   // OPERATORS (39) //
parser[en.Operators] = new SKN()
parser[en.Prove] = new SKN()
...
// MD5('OnError') //
parser['44CDEB54C6F2AEBAD54611201C26D6F0']=function(){ var j = arguments[0]; var k = arguments[1];  var u = arguments[2]; var v = arguments[3]; throw 'Euclid Syntax: *** Error *** - Line '+FMT(j)+' col '+FMT(k)+': Unexpected token ( '+u+' ) - Expected '+v } // MD5('OnError') //
...
addEventListener('message', my_func_syntax,'logic.js')

function my_func_syntax () { 
   var err = false;
   ...
   try{
      buff.map(function(r,i){
         ...
         r.map(function(v,j,me){
            if(j<(J-1) && !Comment.isactivated()){
               ...
               {
                  throw parser['44CDEB54C6F2AEBAD54611201C26D6F0'](i,j,v.literal,ErrMSG[EXIT_FAILURE])
               }
            }
            return v
         })
      ...
      if(EXIT_FAILURE){
         throw parser['44CDEB54C6F2AEBAD54611201C26D6F0'](0,0,'????',EXIT_FAILURE)
      }
   } catch (e) {
     err = true
     buff = e

...
function OnError(e)
{
   return 'line: '+e.lineNumber+'.'+e.columnNumber+': '+e.message+'\nStack trace..\n'+e.stack
}
...
] // NOTE: Only Idnums are directly inserted into the DiGraph
...
var parser = {
'7F2DB423A49B305459147332FB01CF87':{}, // extant memory buffer for incoming tokens
'44CDEB54C6F2AEBAD54611201C26D6F0':function(){ var j = arguments[0]; var k = arguments[1]; throw 'Euclid parser: *** Error *** - Line 0000.'+FMT(j)+': Unexpected token ( '+j+' ) - Expected '+k } // OnError
}
parser['7F2DB423A49B305459147332FB01CF87'] = includes
...
var OUT_STACK_MID = "){ throw parser['44CDEB54C6F2AEBAD54611201C26D6F0'](peek.tok,k.join(' or ')) } else { "
...
var CODE_LOWER_BODY  = "],length:1 }; if(!state00[peek.tok]){ var k = []; for(var expected in state00){ if(expected != 'length') { k.push(expected || '????' ); } } throw parser['44CDEB54C6F2AEBAD54611201C26D6F0'](peek.tok,k.join(' or ')) } proofStack.push(peek.val); }"
var CODE_BASIC = "{ var j = ++arguments[0]; var peek = parser['7F2DB423A49B305459147332FB01CF87'][j]; console.log(j,peek); proofStack.push(peek.val) }"
var CODE_NULL = "{ var j = ++arguments[0]; var peek = parser['7F2DB423A49B305459147332FB01CF87'][j]; proofStack.push(peek.val); return j }"
...
function prover()
{
   this.proof_staus = 'Proof failed.. (current progress)' // (report closest Hamming)
   this.lhs
   this.rhs
   ...
   this.parseDiGraph = function(){
      ...
      try{
         proofStack.push(buffer[0].val) // init'z the stack
         buffer.map(function(v,i,me){
            if (i<me.length-1)
            {
               if(parser[buffer[i].tok])
               {
                  parser[buffer[i].tok](i)
                  return v
               }
               else
               {
                  parser['44CDEB54C6F2AEBAD54611201C26D6F0'](buffer[i].tok,parser['7F2DB423A49B305459147332FB01CF87'][i].tok)
                  result = false
               }
            }
         })
         ...
         if(EXIT_FAILURE){
            parser['44CDEB54C6F2AEBAD54611201C26D6F0']('non'+EXIT_FAILURE,EXIT_FAILURE)
            result = false
         }
         else
         {
   ...
   this.buildDiGraph = function(){
      ...
      this.axioms.map(function(curVertex,i,me){
         if(i<me.length-1){
            ...
            if(!parser[curVertex.tok]){
               ...
               n++
               newSection = codebody.replace(/length:\d+/,inQuotes(nextVertex.tok)+':parser['+inQuotes(nextVertex.tok)+'],length:'+n)
   ...
   this.verifyAxioms = function(){
      try{
         var proof = []
         var parseDG = this.buildDiGraph()
         if(parseDG && this.parseDiGraph()){
            this.proof_staus = 'Proof found!'
         } else {
            throw '*** Parsing error *** -  Euclid is unable to build Axiom model. Please review input file.'
         }
         return proof
      } catch(e) {
         if(e.message){
   ...
   this.prove = function(statements){
      ...
      proof.unshift(this.proof_staus)
      return proof
   }
}
...
function __import(e)
{
   ...
   console.log(proof)
   //postMessage ( { origin:'euclid',value:proof } )
}

/*
var file01 = "lexer.js.00"
var lexer = new Worker(file01)
lexer.addEventListener('message',__import,'logic.js')
*/

function my_func()
{
   //lexer.postMessage(arguments[0].data)
   __import({ data:{ value:arguments[0].data }})
}

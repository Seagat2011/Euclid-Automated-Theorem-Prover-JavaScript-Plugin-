    ...
    if(u.source.startsWith("axiom") && (u.source != self._guid) && u.indir.match(/Reduce|Auto/)){
      ...
      !(val in self._history)
        ...
        tmp.map(function(tok,idx,me){
          ...
          //if(tok == alhs[jdx]){
            if(self._scope_satisfied(tok,me,idx,alhs,jdx)){//if((tok != "{") || ((tok == "{") && (self._scope_satisfied("}",me,idx+1,alhs,jdx+1)))){ // preserve scope //
              vkeys.push(idx)
              if(++jdx==alhs.length){
                g_code._passRound()
    ...
    if(u.source.startsWith("axiom") && (u.source != self._guid) && u.indir.match(/Expand/)){
      ...
      !(val in self._history)
        ...
        tmp.map(function(tok,idx,me){
          ...
          //if(tok == arhs[jdx]){
            if(self._scope_satisfied(tok,me,idx,arhs,jdx)){//if((tok != "{") || ((tok == "{") && self._scope_satisfied("}",me,idx+1,arhs,jdx+1))){ // preserve scope //
              vkeys.push(idx)
              if(++jdx==arhs.length){
                self._subnetFOUND = true
                vkeys.map(function(kdx){
